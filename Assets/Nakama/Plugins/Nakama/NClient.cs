...
namespace Nakama
{
    public class NClient : INClient
    {
        ...
        public Action<INSpacePresence> OnSpacePresence { get; set; }

        public Action<INNotification> OnNotification { get; set; }

        public Action<INEvent> OnEvent { get; set; }

        public Action<INEvent> OnAuthoritateEvent { get; set; }

        public Action<INControlData> OnControlData { get; set; }
        ...
        private void onMessage(Envelope message)
        {
            ...
            switch (message.PayloadCase)
            {
                ...
                case Envelope.PayloadOneofCase.TopicPresence:
                    ...
                    return;
                case Envelope.PayloadOneofCase.SpacePresence:
                    if (OnSpacePresence != null)
                    {
                        OnSpacePresence(new NSpacePresence(message.SpacePresence));
                    }
                    return;
                case Envelope.PayloadOneofCase.EntitiesSyncAck:
                    return;
                case Envelope.PayloadOneofCase.LiveNotifications:
                    ...
                    return;
                case Envelope.PayloadOneofCase.ControlDataMessage:
                    if (OnControlData != null)
                    {
                        OnControlData(new NControlData(message.ControlDataMessage.ControlData));
                    }
                    return;
                case Envelope.PayloadOneofCase.EventMessage:
                    INEvent evt = null;
                    
                    switch (message.EventMessage.Event.EventCase)
                    {
                        case Event.EventOneofCase.MoveEvent:
                            evt = new NMoveEvent(message.EventMessage.Event.MoveEvent);
                            break;
                        case Event.EventOneofCase.CollideEvent:
                            evt = new NCollideEvent(message.EventMessage.Event.CollideEvent);
                            break;
                        case Event.EventOneofCase.CustomEvent:
                            evt = new NCustomEvent(message.EventMessage.Event.CustomEvent);
                            break;
                        case Event.EventOneofCase.ReadyEvent:
                            evt = new NReadyEvent(message.EventMessage.Event.ReadyEvent);
                            break;
                    }

                    switch (message.EventMessage.Audience)
                    {

                        case Audience.MasterClient:
                            if (OnAuthoritateEvent != null){
                                OnAuthoritateEvent(evt);
                            }
                            break;
                        default:
                            if (OnEvent != null){
            ...
            if (!collationIds.ContainsKey(collationId)){
                return;
            }
            ...
            switch (message.PayloadCase)
            {
                case Envelope.PayloadOneofCase.SpaceJoinAck:
                    var sjAcks = new List<INSpaceJoinAck>();
                    sjAcks.Add(new NSpaceJoinAck(message.SpaceJoinAck));
                    pair.Key(new NResultSet<INSpaceJoinAck>(sjAcks, null));
                    break;
                ...
                case Envelope.PayloadOneofCase.Groups:
                    ...
                    break;
                case Envelope.PayloadOneofCase.Spaces:
                    var spaces = new List<INSpace>();
                    foreach (var space in message.Spaces.Spaces)
                    {
                        spaces.Add(new NSpace(space));
                    }
                    pair.Key(new NResultSet<INSpace>(spaces, new NCursor(message.Spaces.Cursor)));
                    break;
                case Envelope.PayloadOneofCase.Assets:
                    var assets = new List<INAsset>();
                    foreach (var asset in message.Assets.Assets)
                    {
                        assets.Add(new NAsset(asset));
                    }
                    pair.Key(new NResultSet<INAsset>(assets, new NCursor(message.Assets.Cursor)));
                    break;
                case Envelope.PayloadOneofCase.SpaceAssets:
                    var spaceAssets = new List<INAsset>();
                    foreach (var asset in message.SpaceAssets.Assets)
                    {
                        spaceAssets.Add(new NAsset(asset));
                    }
                    pair.Key(new NResultSet<INAsset>(spaceAssets, null));
                    break;
                case Envelope.PayloadOneofCase.Avatars:
                    var avatars = new List<INAvatar>();
                    foreach (var avatar in message.Avatars.Avatars)
                    {
                        avatars.Add(new NAvatar(avatar));
                    }
                    pair.Key(new NResultSet<INAvatar>(avatars, new NCursor("")));
                    break;
                case Envelope.PayloadOneofCase.SpaceUsers:
                    var spaceUsers = new List<INUser>();
                    foreach (var spaceUser in message.SpaceUsers.Users)
                    {
                        spaceUsers.Add(new NUser(spaceUser));
                    }
                    pair.Key(new NResultSet<INUser>(spaceUsers, new NCursor("")));
                    break;
                case Envelope.PayloadOneofCase.Entities:
                    var entities = new List<INEntity>();
                    foreach (var entity in message.Entities.Entities)
                    {
                        entities.Add(new NEntity(entity));
                    }
                    pair.Key(new NResultSet<INEntity>(entities, new NCursor(message.Entities.Cursor)));
                    break;
