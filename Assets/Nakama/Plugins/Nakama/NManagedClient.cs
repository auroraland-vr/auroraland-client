...
namespace Nakama
{
    ...
    public class NManagedClient : INClient
    {
        ...
        public Action<INSpacePresence> OnSpacePresence {
            get {
                return _client.OnSpacePresence;
            }
            set {
                _client.OnSpacePresence = (INSpacePresence presence) => {
                    Enqueue(() => value(presence));
                };
            }
        }

        public Action<INControlData> OnControlData {
            get {
                return _client.OnControlData;
            }
            set {
                _client.OnControlData = (INControlData controlData) => {
                    Enqueue(() => value(controlData));
                };
            }
        }

        public Action<INEvent> OnEvent {
            get {
                return _client.OnEvent;
            }
            set {
                _client.OnEvent = (INEvent evt) => {
                    Enqueue(() => value(evt));
                };
            }
        }

        public Action<INEvent> OnAuthoritateEvent {
            get {
                return _client.OnAuthoritateEvent;
            }
            set {
                _client.OnAuthoritateEvent = (INEvent evt) => {
                    Enqueue(() => value(evt));
                };
            }
        }
